...
5. 默认使用 `analyze` 的 `compact` 输出，优先读取 `judgement_hierarchy`、`steps` 和 `evidence`；依据不足时用 `analyze --view full` 查看 `evidence_plan`、`search_query_layers`，或按命盘核心要素调用 `search` 补充，不要只检索一次就下结论。

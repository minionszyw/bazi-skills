...
3. 信息完整后调用 `paipan`；仅当用户问题明确涉及某个日期或时段（如"今年5月事业运"、"今天运势"）时，从问题中提取日期并传入 `--date YYYY-MM-DD`。
4. 按意图选择 `analyze --topic`：未指定用 `overall`；事业 `career`；财运 `wealth`；婚姻 `marriage`；健康 `health`；学业 `study`；父母 `parents`；子女 `children`；兄弟朋友 `siblings`；人际合作 `social`；如何提升/改善/化解/趋避用 `remedy`。
5. 优先使用 `analyze` 输出的 `judgement_hierarchy`、`evidence` 和 `evidence_plan`；依据不足时再按 `search_query_layers` 或命盘核心要素调用 `search` 补充，不要只检索一次就下结论。
...
- `search_query_layers.required`：月令、日主、旺衰、用神、格局等核心依据，用于人工补充检索。
- `search_query_layers.topic_specific`：事业、财运、婚姻、健康等专题依据，用于人工补充检索。
- `search_query_layers.optional`：冲合刑害、大运流年等补充依据，用于人工补充检索。
...
排盘与分析后，先使用 `analyze.evidence` 和 `steps[].evidence_queries`；必要时根据 `search_query_layers` 多轮检索，并结合命盘结果补充关键词：

...
**任何命理问题，包括基于已有命盘的后续追问，必须调用 `bazi ask` 或 `paipan`/`analyze`/`search` 工具链，不得凭自有知识作答。**

本 skill 用于调用本地八字命理工具链，完成出生信息排盘、结构化命理分析、古籍原文检索与中文综合说明。优先使用编排 CLI `bazi ask`；高级调试时再直接使用 `paipan`、`analyze`、`search`。
...
- 需要串联时，优先使用 `bazi ask --save-chart` 或把 `paipan` 输出保存为 JSON 文件，再交给 `analyze --chart`。
- 需要精简结果时，使用 `bazi ask`、`analyze --view compact` 或 `analyze --format text`，不要自行写脚本抽取字段。
- 只有 `paipan | analyze --chart -` 这种 CLI 到 CLI 的直接串联可以使用；在安全策略严格的环境中，也优先使用临时文件。

## 编排入口

用途：调用 `bazi ask`，根据用户自然语言问题自动判断是否需要出生信息、是否需要今日日期、应使用哪个分析主题。

CLI 示例：

```bash
bazi ask --question "请你帮我进行八字命理分析"
bazi ask --question "分析我的财运" --chart /tmp/bazi_current_chart.json
bazi ask --question "如何提升财运" --chart /tmp/bazi_current_chart.json
bazi ask --question "今日运势分析" --chart /tmp/bazi_current_chart.json
```

首次信息完整时：

```bash
bazi ask --question "<用户问题>" --name "<姓名>" --gender <1|0> \
  --calendar <SOLAR|LUNAR> --birth "YYYY-MM-DD HH:MM:SS" \
  --location "<出生地>" --save-chart /tmp/bazi_current_chart.json
```
...
```bash
paipan ... > chart.json
analyze --chart chart.json --intent career
...
- `--intent`：常用意图，例如 `overall`、`wealth`、`improve-wealth`，会自动映射 topic/focus。
